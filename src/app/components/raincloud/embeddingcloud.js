...
export default function EmbeddingCloud() {
    ...
    useEffect(() => {
        ...
        svg.append("text")
            ...
            .text("Embedding Distribution & Variance");

        get_scores().then(function(rawData) {
            if (!rawData || rawData.length === 0) return;

            const categories = [
                { key: 'first_embedding', label: 'hugging-quants', color: '#69b3a2' },
                { key: 'second_embedding', label: 'Qwen', color: '#404080' },
                { key: 'third_embedding', label: 'openai', color: '#f8b195' }
            ];
            ...
            allDensities.forEach((cat, i) => {
                svg.append("path")
                    ...
                    .on("mouseover", function() {
                        ...
                        d3.selectAll(`.stat-${cat.key}`).style("fill", "#000").style("font-weight", "bold");
                    })
                    .on("mouseleave", function() {
                        d3.selectAll("path").style("opacity", "0.3");
                        d3.selectAll(`.stat-${cat.key}`).style("fill", "#666").style("font-weight", "normal");
                    });
